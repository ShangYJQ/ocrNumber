...
if __name__ == "__main__":
    ...
    for i, (x, y) in enumerate(
        iter(test_dataset)
    ):  # use iter() to return a iterator,if you dont add it,pyright will return a error
        output = model(
            x.unsqueeze(0)
        )  # Forward pass: add batch dimension and compute the model output
        predict = output.argmax(
            1
        ).item()  # Get the index of the highest score as the predicted label
        if predict == y:
            right += 1  # Increment the counter if the prediction is correct
        else:
            img_path = test_dataset.samples[i][
                0
            ]  # Get the path of the misclassified image
    ...
    acc = (
        right * 1.0 / sample_num
    )  # Calculate the accuracy as the ratio of correct predictions
